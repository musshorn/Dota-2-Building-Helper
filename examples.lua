--[[
	...
	If player heroes change throughout the game do BuildingHelper:AddUnit(unit) on it after the hero change.
...
function getBuildingPoint(keys)
	...
	if point ~= -1 then
		...
		farm:SetHullRadius(64)-farm:GetCollisionPadding()
...
function getHardFarmPoint(keys)
	...
	if point == -1 then
		...
		farm:SetHullRadius(128)-farm:GetCollisionPadding()
